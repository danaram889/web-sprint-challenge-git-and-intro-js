...
function removeArtist(artists, index) {
  ...
  artists.splice(artists[index], 1);
  var artistsRemoved = artists; 
  return artistsRemoved; 
}
...
function addArtist(artists, name, years, genre, nationality, bio) {
  ...
  var obj = {name, years, genre, nationality, bio}; 
  obj = { 
    name: name, 
    years: years, 
    genre: genre, 
  ...
  updatedArtists.push(obj);
  return updatedArtists; 
  /*Your Code Here*/
}

console.log("Task 6", addArtist(artists, 'John Doe', '1988-2022', 'Full Stack Development', 'African American', 'I have a background in customer service at Big Retail Chain'));
...
function lotsOfArt(artists) {
  ...
  if (artists[i].paintings > 100) {
    artistsNames2.push(artists[i].name);
    }
  }
...
function artistByCountry(artists, nationality){
    var artistsByNationality = []; 
    for (let i = 0; i < artists.length; i++) { 
      if (artists[i].nationality === nationality) { 
        artistsByNationality.push(artists[i].name);
      }
    }
    return artistsByNationality;
